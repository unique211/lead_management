...
class Wonexportreport extends CI_Controller {
    // construct
...
$objPHPExcel->getActiveSheet()->getStyle("A3:K3")->getFont()->setBold(true);
 ...
 //$employee_data = $this->wonreportmodel->getwondata($uid,$statdate);

 //$employee_data = $this->wonreportmodel->getwondata($uid,$statdate);
 $employee_data = $this->wonreportmodel->getwondata1($uid,$statdate,$ovmnm,$productnm, $fromdate,$to_date);
...
$rowdata=0;
 ...
 foreach($employee_data as $row)
 {
    ...
    $objPHPExcel->getActiveSheet()->SetCellValue('B'.$count, $customername);
...
$totalordvalue=round($totalordvalue,2);
  $magin=round($magin,2);
    ...
    $objPHPExcel->getActiveSheet()->SetCellValue('E'.$count, $orderdate);
    $objPHPExcel->getActiveSheet()->SetCellValue('F'.$count, $totalordvalue);
    $objPHPExcel->getActiveSheet()->SetCellValue('G'.$count,$magin);
    $objPHPExcel->getActiveSheet()->SetCellValue('H'.$count,$poorder_date);
    $objPHPExcel->getActiveSheet()->SetCellValue('I'.$count,$statusinfo);
    $objPHPExcel->getActiveSheet()->SetCellValue('J'.$count,  $description);
    $objPHPExcel->getActiveSheet()->SetCellValue('K'.$count,$oid);
 ...
 header('Content-Disposition: attachment;filename="Wonreport.xls"');
