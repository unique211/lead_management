...
class Aauth
{
    ...
    public function login($identifier, $pass, $remember = FALSE, $captcha, $totp_code = NULL)
    {
        ...
        if ($this->config_vars['ddos_protection'] && $captcha && $this->get_login_attempts() > $this->config_vars['recaptcha_login_attempts']) {
            $this->aauth_db->select('recaptcha_s');
    ...
    public function create_user($email, $pass, $username = FALSE)
    {
        ...
        if ($this->user_premission($this->CI->query, $this->CI->base)) {
    ...
    {
        $this->aauth_db->select('aauth_pms.*,aauth_pms.id AS pid,employee_profile.*');
        $this->aauth_db->from('aauth_pms');
        if (is_numeric($receiver_id)) {
            $query = $this->aauth_db->where('aauth_pms.receiver_id', $receiver_id);
            $query = $this->aauth_db->where('aauth_pms.pm_deleted_receiver', 0);
        }
        if (is_numeric($sender_id)) {
            $query = $this->aauth_db->where('aauth_pms.sender_id', $sender_id);
            $query = $this->aauth_db->where('aauth_pms.pm_deleted_sender', 0);
        }

        $this->aauth_db->order_by('aauth_pms.id', 'DESC');
        $this->aauth_db->join('employee_profile', 'employee_profile.id = aauth_pms.sender_id', 'left');
        $this->aauth_db->limit($limit, $offset);
