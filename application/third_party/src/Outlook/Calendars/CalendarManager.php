...
class CalendarManager
{
     ...
     */
    public function all($calid)
    {
        //$response = $this->api->call('/me/calendarview?startDateTime=2020-01-01T00:00:00&endDateTime=2020-12-01T00:00:00&$select=Subject,Organizer,Start,End', 'get');
        $response = $this->api->call("/me/calendars/$calid/events", 'get');
     ...
     * @return array|Calendar
     */
    public function create(Calendar $event,$calenderid)
    {
        ...
        $response = $this->api->call("/me/calendars/{$calenderid}/events", 'post', $event->toParams());
      
       return $this->parseEvents($response);
    }

    public function create1(Calendar $event)
    {
        if ($event->id) {
            return $this->get($event->id);
        }
